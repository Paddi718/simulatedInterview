...
function SessionContent() {
  ...
  const [transcript, setTranscript] = useState('');        // final ASR result
  const [partialTranscripts, setPartialTranscripts] = useState<string[]>([]); // real-time segments
  ...
  const startPcmCapture = useCallback(async (): Promise<boolean> => {
    try {
      ...
      // ScriptProcessorNode: bufferSize=4096 → 256ms @ 16kHz
  ...
  // Display transcript: use final if available, else combine partial segments
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`w-2 h-2 rounded-full ${wsConnected ? 'bg-green-500' : 'bg-gray-300'}`} title={wsConnected ? '语音服务已连接' : '语音服务未连接'} />
            ...
            {phase === 'question' && !isRecording && (
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  <button onClick={handleSkip} disabled={submitState === 'submitting'} className="flex-1 sm:flex-none px-6 py-2.5 text-sm font-medium text-gray-500 bg-gray-50 border border-gray-200 rounded-xl hover:bg-gray-100 hover:text-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    跳过此题
                  </button>
                </div>
              </div>
            ...
            {isRecording && (
              <div className="flex flex-col items-center gap-4">
                ...
                {partialTranscripts.length > 0 && (
                  <div className="w-full bg-gray-50 rounded-xl p-3 border border-gray-200 max-h-40 overflow-y-auto">
                    <p className="text-xs text-gray-400 mb-2">实时转写：</p>
                ...
                <button onClick={handleSkip} disabled={submitState === 'submitting'} className="px-6 py-2 text-sm text-gray-400 border border-gray-200 rounded-xl hover:text-gray-600 hover:border-gray-300 transition-colors disabled:opacity-50">
                  跳过此题
                </button>
              </div>
            )}
            ...
            {phase === 'review' && !isRecording && (
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    onClick={() => submitAnswer(displayTranscript, false)}
                    disabled={submitState === 'submitting' || asrProcessing}
                  ...
                  <button onClick={startRecording} disabled={submitState === 'submitting' || asrProcessing} className="flex-1 sm:flex-none px-6 py-3 text-sm font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-xl hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    重新录音
                  </button>
                  <button onClick={handleSkip} disabled={submitState === 'submitting'} className="flex-1 sm:flex-none px-6 py-3 text-sm font-medium text-gray-400 border border-gray-200 rounded-xl hover:text-gray-600 hover:border-gray-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    跳过
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 sm:p-8 text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} disabled={completing} className="flex-1 px-5 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors disabled:opacity-50">
                再看看
              </button>
              <button onClick={handleComplete} disabled={completing} className="flex-1 px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
